...
import {
  purgeExpiredReportEvidence,
  type AdminEvidencePurgeActionState,
} from "@/app/(protected)/admin/maintenance/actions";

const initialState: AdminEvidencePurgeActionState = {
  outcome: "idle",
  revision: 0,
  selected: 0,
  processed: 0,
...
function describeResult(state: AdminEvidencePurgeActionState) {
  const stoppedCount = state.unavailable + state.failed + state.unknown;
  const unattemptedCount = Math.max(
    0,
    state.selected - state.processed - stoppedCount,
  );

  if (state.outcome === "success") {
    return `選択した${state.selected}件のうち、${state.processed}件の処理が完了しました。`;
  }

  if (state.outcome === "empty") {
    return "実行時点で、期限切れの通報証拠はありませんでした。";
  }

  if (state.outcome === "changed") {
    return "メンテナンス対象が更新されています。最新の件数を確認してください。";
  }

  if (state.outcome === "partial") {
    return `選択した${state.selected}件のうち、${state.processed}件の処理が完了し、現在処理できない対象が${state.unavailable + state.failed}件あったため停止しました。後続の${unattemptedCount}件は実行していません。`;
  }

  if (state.outcome === "unknown") {
    return `選択した${state.selected}件のうち、${state.processed}件の処理が完了しました。結果を確認できない処理が${state.unknown}件あったため、そこで停止し、後続の${unattemptedCount}件は実行していません。繰り返し実行せず、最新の状況を確認してください。`;
  }

  if (state.outcome === "error") {
    return "通報証拠のメンテナンスを実行できませんでした。最新の状況を確認して、もう一度お試しください。";
  }

  return null;
}

export function AdminMaintenanceEvidencePurge({
  batchSize,
  dueCount,
  oldestDueAt,
  oldestDueLabel,
  ...
  const hasDueEvidence = summaryAvailable && dueCount > 0;
  ...
  return (
    <section
      aria-labelledby="evidence-maintenance-heading"
      className="mt-5 min-w-0 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6"
    >
      <p className="text-sm font-semibold text-orange-700">通報証拠</p>
      <h2
        className="mt-1 break-words text-xl font-bold text-stone-800 [overflow-wrap:anywhere]"
        id="evidence-maintenance-heading"
      >
        保持期限を過ぎた証拠
      </h2>
      <p className="mt-3 text-sm leading-6 text-stone-600">
        対応完了から30日を過ぎたsnapshotと証拠画像の関連情報を削除します。通報本体と画像の物理データはこの操作では削除しません。
      </p>
      ...
      {isConfirming ? (
        ...
        >
          ...
          >
            期限を過ぎた通報証拠を最大{batchSize}
            件削除します。削除した証拠は元に戻せません。
          </p>
        ...
        <button
          ...
          aria-disabled={
            isPending || !hasDueEvidence || mustReloadAfterUnknown
          }
          ...
          disabled={isPending || !hasDueEvidence || mustReloadAfterUnknown}
