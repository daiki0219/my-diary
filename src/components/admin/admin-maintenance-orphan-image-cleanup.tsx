...
import {
  cleanupUnconfirmedExchangeImageOrphans,
...
function describeResult(state: AdminExchangeImageCleanupActionState) {
  ...
  if (state.outcome === "success") {
    return `選択した${state.selected}件のうち、${state.processed}件の物理削除が完了しました。`;
  }

  if (state.outcome === "empty") {
    return "実行時点で、期限を過ぎた未確定画像はありませんでした。";
  }
  ...
  if (state.outcome === "partial") {
    return `選択した${state.selected}件のうち、${state.processed}件の物理削除が完了しました。現在処理できない対象が${state.unavailable + state.failed}件あったため停止し、後続の${unattemptedCount}件は実行していません。`;
  }

  if (state.outcome === "unknown") {
    return `選択した${state.selected}件のうち、${state.processed}件の物理削除を確認しました。結果を確定できない処理が${state.unknown}件あったため停止し、後続の${unattemptedCount}件は実行していません。繰り返し実行せず、最新の状況を確認してください。`;
  }

  if (state.outcome === "error") {
    return "未確定画像のメンテナンスを実行できませんでした。最新の状況を確認して、もう一度お試しください。";
  }

  return null;
}

export function AdminMaintenanceOrphanImageCleanup({
  batchSize,
  dueCount,
  oldestDueAt,
  oldestDueLabel,
  ...
  return (
    <section
      aria-labelledby="orphan-image-maintenance-heading"
    ...
    >
      ...
      <h2
        ...
        id="orphan-image-maintenance-heading"
      >
        entryへ未確定の古い画像
      </h2>
      <p className="mt-3 text-sm leading-6 text-stone-600">
        upload後24時間以上entryへ確定されなかった画像を、現在の参照状態を再確認してStorageから物理削除します。
      </p>
      ...
      {isConfirming ? (
        ...
        >
          ...
          >
            entryへ未確定の古い画像を物理削除しますか？
          </h3>
          <p
            className="mt-2 break-words text-sm leading-6 text-stone-700 [overflow-wrap:anywhere]"
            id={confirmationDescriptionId}
          >
            upload後24時間以上entryへ確定されなかった画像を最大{batchSize}
            件処理します。現在の参照状態を再確認してからStorageから削除します。削除した画像は元に戻せません。
          </p>
      ...
      <p aria-atomic="true" aria-live="polite" className="sr-only" role="status">
        {isPending ? "未確定画像のメンテナンスを実行しています" : ""}
      </p>
    </section>
  );
}
